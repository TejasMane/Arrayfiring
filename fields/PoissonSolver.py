...
def SOR(rho_with_ghost, ghost_cells, dx, dy, *args, **kwargs):
    ...
    V_k_plus[X_physical_index, Y_physical_index] =  (1-omega) * V_k[X_physical_index, Y_physical_index] \
                                                    + (omega/(2*dx**2 + 2*dy**2)) \
                                                    * (    (dy**2)*(V_k[X_physical_index, Y_physical_index + 1] + V_k[X_physical_index, Y_physical_index - 1]) \
                                                        + (dx**2)*(V_k[X_physical_index + 1, Y_physical_index] + V_k[X_physical_index - 1, Y_physical_index]) \
                                                        + ((dx**2)*dy**2)*(rho_with_ghost[X_physical_index, Y_physical_index]) \
                                                      )

    # print('Before loop Vk plus ', V_k_plus)
    # print('Before loop af.max(af.abs(V_k_plus - V_k)) ',af.max(af.abs(V_k_plus - V_k)) )
    while(af.max(af.abs(V_k_plus - V_k)) > epsilon):
        # print(' \n iteration \n')
        V_k = V_k_plus.copy()
